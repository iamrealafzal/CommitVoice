...
Usage examples:
    ...
    # Filter by date
    python tools/social_updates.py --since "2024-01-01"
"""
...
def clone_repo(remote: str, branch: str, depth: int) -> Path:
    ...
    if code != 0 and branch != "master":
        # Try master as a fallback when main is not present.
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--remote",
        default="git@bitbucket.org:afzal--lakdawala/shopycart.git",
        help="Remote git URL to read commits from.",
    )
    parser.add_argument(
        "--branch",
        default="main",
        help="Branch to pull commits from (falls back to master if missing).",
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=5,
        help="Number of commits to include.",
    )
    parser.add_argument(
        "--since",
        help='Only include commits after this date (e.g., "2024-01-01").',
    )
    parser.add_argument(
        "--until",
        help='Only include commits up to this date (e.g., "2024-12-31").',
    )
    parser.add_argument(
        "--depth",
        type=int,
        default=20,
        help="Shallow clone depth; higher if commits are older than this window.",
    )
    parser.add_argument(
        "--gemini-model",
